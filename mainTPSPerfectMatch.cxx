/*
File:			mainTPSPerfectMatch.cxx

Description:	Subfunction called from imageioraw.cxx applies a displacement field onto a set of points and generates a new displacement field at a different image resolution compared to the input displacement field using thin plate spline interpolation. Input parameters based on input processing from calling mainTPSPerfectMatch as a command line function, ignoring the first 3 input parameters. Refer below for input process parameters.

Inputs:			argc - (int) Number of input arguments stored in argv. Default number is 10.
				argv[2]: inputVectorField1 - (std::string) Path name of the raw data file of the displacement field. Pixels are in 32 bit float format.
				argv[3]: inVectorConfigFile1 - (std::string) Path name of the configuration text file of the displacement field. Pixels are in 32 bit float format.
			
				argv[4]: inPointsFile - (std::string) Path name of the matched point set for the input image.
...
bool mainTPSPerfectMatch (int argc, char *argv[]) {
	...
	} else
	{
		...
		std::cerr << "--TPSPerfectMatch inputVectorField1 inVectorConfigFile1 inPointsFile outPointsFile outputDisplacementField outDisplacementConfig relativePosition(origin/center) outputDisplacementResolution(original/#)"<< std::endl;
		return EXIT_FAILURE;
	}
	...
	std::cout << "Supposedly, vector field is read in properly." << std::endl;	
	...
	inputKeypoints.setFileName(inPointsFile);
	inputKeypoints.readKeypointSet();
	

	//Transform fixedImageMatchedPoints to their proper location via ideal displacement field

	//Change Vector Field into a Transform Field
	
	typedef itk::DisplacementFieldTransform<PixelType, Dimension>  DeformationFieldTransformType;
	DeformationFieldTransformType::Pointer deformationFieldTransform = DeformationFieldTransformType::New();
	deformationFieldTransform->SetDisplacementField( vectorField );
	
	
	//Transform fixedImageMatchedPoints into their ideal location
	PointSetType::Pointer movingImageMatchedPoints = PointSetType::New();
	PointSetType::PointsContainerPointer tmpPointsContainer = movingImageMatchedPoints->GetPoints();

	for (unsigned int i = 0;  i < fixedImageMatchedPoints->GetNumberOfPoints(); ++i) {
		PointType pp,tmp;
		fixedImageMatchedPoints->GetPoint(i, &tmp);
		pp = deformationFieldTransform->TransformPoint(tmp);
		
		tmpPointsContainer->InsertElement(i, pp);
	}
	...
	if (outputResolution != "original") {
		...
		if (!(iss >> displacementResampleFactor)) {
			...
			inputImageImport5->setConfigFileName(outDisplacementConfig);
