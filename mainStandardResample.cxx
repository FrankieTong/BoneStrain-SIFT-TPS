/*
File:			mainStandardResample.cxx

Description:	Subfunction called from imageioraw.cxx Performs SIFT feature registration on 2 input images. Input parameters based on input processing from calling mainStandardResample as a command line function, ignoring the first 3 input parameters. Refer below for input process parameters.

Inputs:			argc - (int) Number of input arguments stored in argv. Default number is 15.
				...
				argv[11]: resampleFactorStr - (std::string) Downsample factor of the input images before using SIFT. Input is a float number in a string format.
				argv[12]: relativePosition - (std::string) Determines where the origin of the output image/field files are placed. They can either be placed at the "origin" or at the "center" of the image/field. Input is a string containing "origin" or "center.
				argv[13]: outputResolution - (std::string) Downsample factor of the output images/fields after using SIFT. Resample based on original input image resolution. Input is a float number in a string format.
				argv[14]: inRemoveEdgePointsPercent - (std::string) Percentage of image removed starting from the image edge and moving inward based on toral image size in each dimension. Input is a float number [0,100] in string format.
...
bool mainStandardResample (int argc, char *argv[]) {
	...
	if (argc == 15) {
		...
		resampleFactorStr = argv[11];
		relativePosition = argv[12];
		outputResolution = argv[13];
		inRemoveEdgePointsPercent = argv[14];
	}
	...
	else
	{
		...
		std::cerr << " --standardResample inputImageFile1 inputImageFile2 inConfigFile1 inConfigFIle2 outputImageFile outConfgFile outPointsFile outputDisplacementField outDisplacementConfig resampleSpacingFactor relativePosition(origin/center) outputDisplacementResolution(original/resampled/both) inRemoveEdgePointsPercent"<< std::endl;
		return EXIT_FAILURE;
	}
	...
	double removeEdgePointsPercent= 0;
	...
	fixedImageKeyPoints = siftFilter1->getSiftFeatures(resampledFixedImage);
	//delete siftFilter1;

	//SiftFilterType * siftFilter2 = new SiftFilterType;
	movingImageKeyPoints = siftFilter1->getSiftFeatures(resampledMovingImage);
	//delete siftFilter2;
