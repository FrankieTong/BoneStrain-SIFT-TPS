/*
File:			mainStandardResampleScaleAdjust.cxx

Description:	Subfunction called from imageioraw.cxx Performs SIFT feature registration on 2 input images. Input parameters based on input processing from calling mainStandardResampleScaleAdjust as a command line function, ignoring the first 3 input parameters. Refer below for input process parameters.

Inputs:			argc - (int) Number of input arguments stored in argv. Default number is 19.
				...
				argv[17]: inEdgePointRatio - (std::string) Starting edge point ratio value for scale level 0. The higher the number, the more feature points will be accepted for matching. Input is a float number greater than 0. Input can also be a negative number if you want to disregard edge point checking for feature points. If you are unsure about this value, leave it at 10.
				argv[18]: inEdgePointRatioScale - (std::string) Edge point ratio multiplier that multiplies the edge point ratio by this number each time SIFT searches up a higher scale. Input is a float number. Default is 1 for no adjustment in the edge point ratio between scales.
...
bool mainStandardResampleScaleAdjust (int argc, char *argv[]) {
	...
	if (argc == 17) {
		...
		inRemoveEdgePointsPercent = argv[16];
		inEdgePointRatio = argv[17];
		inEdgePointRatioScale = argv[18];
	} else
	{
		std::cerr << "Incorrect number of parameters " << std::endl;
		std::cerr << "Usage: " << argv[0];
		std::cerr << " --standardResampleScaleAdjust inputImageFile1 inputImageFile2 inConfigFile1 inConfigFile2 (inImageFile1Min) (inImageFile1Max) (inImageFile2Min) (inImageFile2Max) outPointsFile outputDisplacementField outDisplacementConfig resampleSpacingFactor relativePosition(origin/center) outputDisplacementResolution(original/resampled/both/#) inRemoveEdgePointsPercent inEdgePointRatio[optional] inEdgePointRatio[optional]"<< std::endl;
		return EXIT_FAILURE;
	}

	float resampleFactor = 0;
	...
	fixedImageKeyPoints = siftFilter1->getSiftFeatures(resampledFixedImage, edgePointRatio, edgePointRatioScale);
	//delete siftFilter1;

	//SiftFilterType * siftFilter2 = new SiftFilterType;
	movingImageKeyPoints = siftFilter1->getSiftFeatures(resampledMovingImage, edgePointRatio, edgePointRatioScale);
	//delete siftFilter2;
